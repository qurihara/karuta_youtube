...
YouTubeで競技かるたの試合動画を視聴中、「発話前に戻る」ボタンをワンクリックで使えるChrome拡張 (Manifest V3)。

下の句→無音→次の和歌の上の句、というかるた特有の構造を、ブラウザ内のVAD (Silero VAD) でリアルタイム検出し、ユーザーが「上の句が始まったあと」にコントロールUIを押すと、上の句開始の N 秒前 (既定 1秒) まで巻き戻して再生し直す。

通常動画 (`/watch?v=...`) と **YouTube Live** (`/live/<id>` / `/watch?v=...`) の両方に対応。ライブ視聴時はコントロールUIに「LIVE」バッジが表示され、DVRバッファ範囲外への巻き戻しを試みる場合はボタンが自動的にグレーになる。
...
1. YouTubeで動画を開く (`https://www.youtube.com/watch?v=...` または `/live/...`)
   -　たとえば　https://www.youtube.com/watch?v=VJlVguEH3dk&t=1602s
3. 動画上部中央にコントロールUIが現れる (デフォルトでは「◀ 発話前に戻る」ボタンと ⚙ だけ表示)
4. 新たな上の句の開始が検出されるとボタンが**青になりフラッシュ**で点滅
5. クリックすると、上の句開始の N 秒前にseekして再生し直す
6. ⚙ ボタンで詳細パネルが開き、以下を調整・確認できる:
...
7. ツールバーアイコンのポップアップで拡張全体を ON / OFF (緑=ON, グレー=OFF)。OFF にすると音声認識・コントロールUI表示の両方が停止
...
- **コントロールUI**: Shadow DOM。動画上部中央に position: fixed で追従。フルスクリーン切替時は fullscreen 要素内に移動して継続表示
...
DevTools の Console フィルタを `[karuta]` に絞ると見やすい。コントロールUIの ⚙ から見える診断行 (`f:... pk:.../... g:...x p:.../...`) も常時更新されていて、リアルタイム状態の確認に使える。

## 既知の制限

- YouTube が広告を本編と同じ `<video>` 要素で再生するため、広告中の発話も VAD に混じる
- AudioContext の autoplay 制限により、初回再生時にユーザー操作 (動画クリックなど) が必要な場合がある
- **ライブ配信**: 巻き戻し先 (uta開始 − N秒) が `video.seekable.start(0)` を超えている場合は、自動的にバッファの先頭までで頭打ち。コントロールUIのボタンは200ms間隔で「届く範囲か」をチェックし、届かない時はグレーに戻る
